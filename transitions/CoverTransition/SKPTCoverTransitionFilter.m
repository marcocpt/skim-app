...
#import "SKPTCoverTransitionFilter.h"
#import <Foundation/Foundation.h>
#import <ApplicationServices/ApplicationServices.h>

@implementation SKPTCoverTransitionFilter

static CIKernel *_SKPTCoverTransitionFilterKernel = nil;

- (id)init
{
    if(_SKPTCoverTransitionFilterKernel == nil)
    {
		NSBundle    *bundle = [NSBundle bundleForClass:NSClassFromString(@"SKPTCoverTransitionFilter")];
		NSStringEncoding encoding = NSUTF8StringEncoding;
		NSError     *error = nil;
		NSString    *code = [NSString stringWithContentsOfFile:[bundle pathForResource:@"SKPTCoverTransitionFilterKernel" ofType:@"cikernel"] encoding:encoding error:&error];
		NSArray     *kernels = [CIKernel kernelsWithString:code];

		_SKPTCoverTransitionFilterKernel = [[kernels objectAtIndex:0] retain];
    }
    return [super init];
}
...
- (CGRect)regionOf:(int)sampler destRect:(CGRect)R userInfo:(NSNumber *)offset {
    if (sampler == 1) {
        R = CGRectUnion(CGRectOffset(R, [offset floatValue], 0.0), CGRectOffset(R, -[offset floatValue], 0.0));
    }
    return R;
}
...
- (CIImage *)outputImage
{
    ...
    NSNumber *offset = [NSNumber numberWithFloat:[inputExtent Z] * (1.0 - [inputTime floatValue])];
